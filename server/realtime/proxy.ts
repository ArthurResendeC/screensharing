import { z } from 'zod';
import { roomCredentialSchema, roomIdSchema } from '../../src/lib/signaling/messages';
import {
  issueRealtimeTicket,
...
const sessionDescriptionSchema = z
  .object({ type: z.enum(['offer', 'answer']), sdp: z.string().min(1).max(64_000) })
  .strict();
...
const renegotiateSchema = z
  .object({ ticket: z.string().min(1).max(2048), sessionDescription: sessionDescriptionSchema })
  .strict();
...
export function createRealtimeProxy(options: RealtimeProxyOptions) {
  ...
  async function createSession(request: Request): Promise<Response> {
    ...
    const verified = verifyRoomCredential(options.roomTokenSecret, body.roomId, body.credential);
    if (!verified.ok) {
      limiter.fail(key);
    ...
    if (verified.room.passwordProof) {
      const validToken = body.accessToken
        ? verifyRoomAccessToken(options.roomTokenSecret, body.roomId, body.accessToken)
        : null;
      const validPassword = body.password && verifyRoomPassword(options.roomTokenSecret, verified.room, body.password);
      if (!validToken && !validPassword) {
        limiter.fail(key);
        return json({ reason: body.password ? 'wrong-password' : 'password-required' }, 403);
      }
    }
    ...
    return json(
      {
        ...
        ticket: issueRealtimeTicket(options.roomTokenSecret, body.roomId, created.data.sessionId),
  ...
  function sessionOf(ticket: string): string | null {
    return verifyRealtimeTicket(options.roomTokenSecret, ticket)?.sessionId ?? null;
  }

  return async function handle(request: Request): Promise<Response> {
    const path = new URL(request.url).pathname;

    if (request.method === 'POST' && path === '/realtime/session') return createSession(request);
    ...
    if (request.method === 'POST' && path === '/realtime/tracks/new') {
      ...
      return cf(`/sessions/${sessionId}/tracks/new`, 'POST', {
        ...(body.sessionDescription ? { sessionDescription: body.sessionDescription } : {}),
        tracks: body.tracks,
      });
    }
    ...
    if (request.method === 'PUT' && path === '/realtime/renegotiate') {
      ...
      const sessionId = sessionOf(body.ticket);
      if (!sessionId) return json({ reason: 'expired' }, 401);
      return cf(`/sessions/${sessionId}/renegotiate`, 'PUT', { sessionDescription: body.sessionDescription });
    }
