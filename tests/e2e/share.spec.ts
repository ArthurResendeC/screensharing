...
async function identity(page: Page) {
	return (await page.getByText(/^Você: Participante /).innerText()).replace(
		"Você: ",
		"",
	);
}
...
async function playing(page: Page, color: "red" | "blue", audio: number) {
	...
	await expect(
		page.getByText("Conexão: connected", { exact: true }),
	).toBeVisible();
	if (audio) {
		await expect
			.poll(() =>
...
async function cleared(page: Page) {
	await expect
		.poll(() =>
			remoteVideo(page).evaluate(
				(video: HTMLVideoElement) => video.srcObject === null,
			),
		)
		.toBe(true);
}
...
test("five participants: simultaneous publishing, reciprocal watching and one remote stream through switches", async ({
	...
	await expect(
		a.getByText("Participantes: 5 / 5", { exact: true }),
	).toBeVisible();
	await a
		.getByRole("button", { name: "Compartilhar tela", exact: true })
		.click();
	await b
		.getByRole("button", { name: "Compartilhar tela", exact: true })
		.click();
	...
	await expect
		...
		.toEqual({ sending: 0, receiving: 1, total: 1 });
	await c
		.getByRole("button", { name: "Parar de assistir", exact: true })
		.click();
	await cleared(c);
	await expect
		.poll(() => activeCounts(c))
	...
	await expect(
		b.getByText("Participantes: 4 / 5", { exact: true }),
	).toBeVisible();
	await cleared(b);
	await playing(c, "blue", 0);
...
test("signaling disconnect closes both directions and capture; reconnect rejoins the existing room", async ({
	...
	await expect(
		b.getByRole("button", { name: "Reconectar à sala" }),
	).toBeVisible();
	await cleared(a);
	await cleared(b);
	expect(
	...
	await expect(
		b.getByText("Participantes: 2 / 5", { exact: true }),
	).toBeVisible();
