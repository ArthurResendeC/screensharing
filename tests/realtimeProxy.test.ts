...
type CfCall = { url: string; method: string; auth: string | null; body: unknown };
let cfCalls: CfCall[] = [];
let cfResponder: (call: CfCall) => Response = () => Response.json({ sessionId: 'cf-sess-1' }, { status: 201 });

beforeEach(() => {
  cfCalls = [];
  cfResponder = () => Response.json({ sessionId: 'cf-sess-1' }, { status: 201 });
...
async function newSession(overrides: Record<string, unknown> = {}, ip?: string) {
  const roomId = crypto.randomUUID();
  const credential = issueRoomCredential(ROOM_SECRET, roomId, 'Sala', overrides.password as string | undefined);
  return {
    roomId,
    credential,
...
test('public room: creates a CF session and returns a ticket + answer', async () => {
  cfResponder = () =>
    Response.json({ sessionId: 'cf-sess-9', sessionDescription: { type: 'answer', sdp: 'v=0\r\n' } }, { status: 201 });
...
test('track calls need a valid ticket and forward to the bound session', async () => {
  cfResponder = () => Response.json({ sessionId: 'cf-sess-42' }, { status: 201 });
  ...
  cfResponder = () =>
    Response.json({ requiresImmediateRenegotiation: false, tracks: [{ mid: '0', trackName: 't' }] }, { status: 200 });
  const ok = await proxy(
    req('/realtime/tracks/new', 'POST', {
      ticket,
  ...
  expect(forwarded.url).toBe('https://rtc.live.cloudflare.com/v1/apps/app-123/sessions/cf-sess-42/tracks/new');
  expect(forwarded.method).toBe('POST');
});

test('renegotiate and tracks/close proxy to the bound session', async () => {
  cfResponder = () => Response.json({ sessionId: 'cf-sess-7' }, { status: 201 });
  const { response } = await newSession({}, '203.0.113.11');
  const { ticket } = (await response.json()) as { ticket: string };

  cfResponder = () => Response.json({}, { status: 200 });
  await proxy(req('/realtime/renegotiate', 'PUT', { ticket, sessionDescription: { type: 'answer', sdp: 'v=0\r\n' } }));
  expect(cfCalls.at(-1)!.url).toBe('https://rtc.live.cloudflare.com/v1/apps/app-123/sessions/cf-sess-7/renegotiate');
  expect(cfCalls.at(-1)!.method).toBe('PUT');

  await proxy(req('/realtime/tracks/close', 'PUT', { ticket, tracks: [{ mid: '3' }], force: true }));
  expect(cfCalls.at(-1)!.url).toBe('https://rtc.live.cloudflare.com/v1/apps/app-123/sessions/cf-sess-7/tracks/close');
});
