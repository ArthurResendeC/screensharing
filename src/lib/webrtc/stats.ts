...
export async function collectStats(pc: RTCPeerConnection, previous: Map<string, Sample>): Promise<VideoStats[]> {
  ...
  report.forEach(
    (
      ...
      const bitrate =
        last && stat.timestamp > last.timestamp && bytes >= last.bytes
          ? ((bytes - last.bytes) * 8000) / (stat.timestamp - last.timestamp)
          : undefined;
      ...
      rows.push({
        ...
        packets: stat.packetsSent ?? stat.packetsReceived,
        bitrate,
        direction: stat.type === 'outbound-rtp' ? 'Enviado' : 'Recebido',
      });
    },
  );
  return rows;
