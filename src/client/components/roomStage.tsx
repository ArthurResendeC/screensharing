...
export function RoomStage({
  ...
  const live = state.members.filter(
    member => member.sharing && member.peerId !== state.selfId,
  );
  ...
  const title = watching
    ? state.selectedIds.map(peerId => controller.nameOf(peerId)).join(' + ')
    : 'Minha transmissão';
    ...
    <div className="main">
      <div className="topbar">
        <button
          ...
          title={
            sidebarCollapsed
              ? 'Expandir painel de participantes'
              : 'Recolher painel de participantes'
          }
          aria-label={
            sidebarCollapsed
              ? 'Expandir painel de participantes'
        ...
        {state.selectedIds.length === 2 && (
          <div
            ...
            role="group"
            aria-label="Layout das transmissões"
          >
            <button
              type="button"
        ...
        <div className="actions">
          ...
          >
            <PlusIcon />{' '}
            {state.inviteCopied ? 'Convite copiado' : 'Copiar convite'}
          </button>
        </div>
      </div>
      <div className="stage">
        ...
        {state.selfId && (
          <p className="info-line">Você: {controller.nameOf(state.selfId)}</p>
        )}
              ...
              {live.length === 0 ? (
                <div className="empty-placeholder">
                  ...
                  <div>
                    ...
                    <p>
                      Assim que alguém compartilhar a tela, ela aparece aqui
                      para você escolher.
                    </p>
                  </div>
                </div>
          ...
          {watching && (
            ...
            >
              {state.selectedIds.map((peerId, index) => {
                const remote = state.remoteStreams.find(
                ...
                return (
                  <div className="remote-stream-cell" key={peerId}>
                    {!remote && (
                      <span className="remote-stream-loading">
                        Conectando a {controller.nameOf(peerId)}…
                      </span>
                    )}
                    <MediaVideo
        ...
        <div className="action-bar">
          ...
          >
            <ScreenIcon size={16} />{' '}
            {state.capturing ? 'Selecionando tela…' : 'Compartilhar tela'}
          </button>
          <button
            type="button"
            className="btn btn-outline"
          ...
          <details className="local-preview">
            ...
            <div className="watchers-row">
              <WatcherList
                ...
                nameOf={peerId => controller.nameOf(peerId)}
              />
            </div>
          </details>
        )}
