...
const NETWORK_ONLY_PATHS = new Set(['/config.json', '/health', '/service-worker.js', '/signaling']);
...
async function cacheLinkedAssets(cache, documentResponse) {
  ...
  for (const match of html.matchAll(LINKED_ASSET_PATTERN)) {
    ...
    if (url.origin === self.location.origin && !NETWORK_ONLY_PATHS.has(url.pathname)) urls.add(url.href);
  }
  await Promise.all(
    [...urls].map(async url => {
...
async function installShell() {
  ...
  if (!documentResponse.ok) throw new Error('Não foi possível armazenar o shell do ReShare.');
  await Promise.all([
...
async function refreshDocument(cache, request, response) {
  await Promise.all([cache.put(request, response.clone()), cacheLinkedAssets(cache, response)]);
}
...
async function updateCache(request, networkResult) {
  try {
    ...
    const cache = await caches.open(CACHE_NAME);
    if (request.mode === 'navigate') await refreshDocument(cache, request, response);
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.method !== 'GET' || url.origin !== self.location.origin || NETWORK_ONLY_PATHS.has(url.pathname))
    return;
  const networkResult = fetch(event.request).then(response => ({ client: response, cache: response.clone() }));
  event.respondWith(networkResult.then(result => result.client).catch(() => cachedResponse(event.request)));
