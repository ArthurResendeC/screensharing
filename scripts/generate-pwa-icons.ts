...
function colorAt(x: number, y: number) {
  const inMonitorStroke =
    roundedRect(x, y, 127, 147, 258, 196, 37) &&
    !roundedRect(x, y, 151, 171, 210, 148, 13);
  const inStand =
    line(x, y, 256, 331, 256, 379, 24) || line(x, y, 202, 379, 310, 379, 24);
    ...
    for (let x = 0; x < size; x++) {
      ...
      for (let sampleY = 0; sampleY < SAMPLES; sampleY++) {
        for (let sampleX = 0; sampleX < SAMPLES; sampleX++) {
          const designX =
            ((x + (sampleX + 0.5) / SAMPLES) * DESIGN_SIZE) / size;
          const designY =
            ((y + (sampleY + 0.5) / SAMPLES) * DESIGN_SIZE) / size;
...
await Promise.all(
  outputs.map(([size, path]) => Bun.write(path, encodePng(size))),
);
